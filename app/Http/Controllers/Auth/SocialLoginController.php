...
class SocialLoginController extends Controller
{
    ...
    private function send_hablame_sms($phone, $message) {
        $ch = curl_init();

        $postFields = array (
            'account' => env('HABLAME_ACCOUNT'),
            'apiKey' => env('HABLAME_API_KEY'),
            'token' => env('HABLAME_TOKEN'),
            'toNumber' => str_replace('+', '', $phone),
            'sms' => $message,
            'isPriority' => 1,
        );

        $api_endpoint = "https://api101.hablame.co/api/sms/v2.1/send/";
    ...
    private function consult_hablame_sms($smsId) {
        $ch = curl_init();
        ...
        $api_endpoint = "https://api101.hablame.co/api/sms/v2.1/query/smsId/";
        curl_setopt( $ch , CURLOPT_URL, $api_endpoint );
        curl_setopt( $ch , CURLOPT_POST, 1 );
        curl_setopt( $ch , CURLOPT_POSTFIELDS, $postFields );
        curl_setopt( $ch , CURLOPT_RETURNTRANSFER, true );
        ...
        try{
            ...
            if ($twilio_enable) {
                ...
                $client->verify->v2->services(env('TWILIO_VERIFY_SID'))
                    ->verifications
                    ->create($request->country . $request->phone, "sms");
            } else {
                $SmsCode = new SmsCode;
                $SmsCode->phone = str_replace(array(' ', ',', '-'), '', $request->country . $request->phone);
                $SmsCode->code = mt_rand(100000 , 999999);
                $response = $this->send_hablame_sms($SmsCode->phone, 'Tu código de verificación para Polar es: ' . $SmsCode->code);
                //$this->consult_hablame_sms($response['smsId']);
                $SmsCode->save();
            }
            // return response()->json($response);
            return response()->json(trans('api.sent_code'));
        }
        catch (Exception $e) {
    ...
    public function twilio_email(Request $request)
    {
        try{
            $twilio_enable = false;
            if ($twilio_enable) {
                ...
                $client->verify->v2->services(env('TWILIO_VERIFY_SID'))
                    ->verifications
                    ->create($request->country . $request->phone, "email");
            } else {
                $SmsCode = new SmsCode;
                $SmsCode->phone = str_replace(array(' ', ',', '-'), '', $request->country . $request->phone);
                $SmsCode->code = mt_rand(100000 , 999999);
                $response = $this->send_hablame_sms($SmsCode->phone, 'Tu código de verificación para Let’s Go Latam es: ' . $SmsCode->code);
                //$this->consult_hablame_sms($response['smsId']);
                $SmsCode->save();
            }
    ...
    public function twilio_code(Request $request) {
        try{
            $twilio_enable = false;
            if ($twilio_enable) {
                $client = new Client(env('TWILIO_SID'), env('TWILIO_AUTH_TOKEN'));

                $verification = $client->verify->v2->services(env('TWILIO_VERIFY_SID'))
                    ->verificationChecks
                    ->create($request->code, array('to' => $request->country . $request->phone));

                if ($verification->valid) {
                    return response()->json(trans('api.verified_code'));
                } else {
                    return response()->json(['status' => false, 'message' => trans('api.invalid_code')], 422);
                }
            } else {
                $SmsCode = SmsCode::where('phone', str_replace(array(' ', ',', '-'), '', $request->country . $request->phone))->where('code', $request->code)->first();
                if ($SmsCode) {
                    $SmsCode->delete();
                    return response()->json(trans('api.verified_code'));
                } else {
