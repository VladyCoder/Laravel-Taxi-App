    ...
    <div class="container-fluid">
        <div class="box box-block bg-white">
            <form action="{{route('admin.settings.payment.store')}}" method="POST">
                ...
                <div class="card card-block card-inverse card-primary">
                    <blockquote class="card-blockquote">
                        <i class="fa fa-3x fa-cc pull-right"></i>
                        <div class="form-group row">
                            <div class="col-xs-4 arabic_right">
                                <label for="epay_secret_key" class="col-form-label">
                                    @lang('admin.payment.epay_payments')
                                </label>
                            </div>
                            <div class="col-xs-6">
                                <input @if(Setting::get('EPAY') == 1) checked  @endif  name="EPAY" id="epay_check" onchange="cardselect()" type="checkbox" class="js-switch" data-color="#43b968">
                            </div>
                        </div>
                        <div id="epay_field" @if(Setting::get('EPAY') == 0) style="display: none;" @endif>
                            <div class="form-group row">
                                <label for="epay_public_key" class="col-xs-4 col-form-label">@lang('admin.payment.epay_public_key')</label>
                                <div class="col-xs-8">
                                    <input class="form-control" type="text" value="{{Setting::get('epay_public_key', '') }}" name="epay_public_key" id="epay_public_key"  placeholder="ePayco public key">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="epay_private_key" class="col-xs-4 col-form-label">@lang('admin.payment.epay_private_key')</label>
                                <div class="col-xs-8">
                                    <input class="form-control" type="text" value="{{Setting::get('epay_private_key', '') }}" name="epay_private_key" id="epay_private_key"  placeholder="ePayco private key">
                                </div>
                            </div>
                            <!-- <div class="form-group row">
                                <label for="stripe_oauth_url" class="col-xs-4 col-form-label">Stripe Oauth Url</label>
...
function cardselect()
{
    ...
    if($('#epay_check').is(":checked")) {
        $("#epay_field").fadeIn(700);
    } else {
        $("#epay_field").fadeOut(700);
    }
}
