...
<script>
    ...
    function addMarkerToMap(element, index) {
        ...
        if (element.service) {
            provider_services = '\n';
            for (var service_index = 0; service_index < element.service.length; service_index++) {
                provider_services += '\n' + element.service[service_index].service_type.name;
            }
        }
        marker = new google.maps.Marker({
            ...
            id: element.id,
            map: map,
            title: element.first_name + " " + element.last_name + provider_services,
            icon : mapIcons[element.service ? (element.service.length ? element.service[0].status : 'unactivated') : element.status],
        });
