@extends('admin.layout.base')

@section('title', 'Request details ')

@section('content')
    <div class="content-area py-1">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                <h4>@lang('admin.request.request_details')</h4>
                <a href="{{ route('admin.adminrequest.index') }}" class="btn btn-primary pull-right">
                    <i class="fa fa-angle-left"></i> @lang('admin.back')
                </a>
                <div class="row">
                    <div class="col-md-6">
                        <dl class="row">
                            <dt class="col-sm-4">@lang('admin.request.Booking_ID') :</dt>
                            <dd class="col-sm-8">{{ $request->booking_id }}</dd>

                            <dt class="col-sm-4">@lang('admin.request.Admin_Name') :</dt>
                            <dd class="col-sm-8">{{ $request->admin->name }}</dd>

                            <dt class="col-sm-4">@lang('admin.request.total_distance') :</dt>
                            <dd class="col-sm-8">{{ $request->distance ? $request->distance : '-' }}{{$request->unit}}</dd>

                            <dt class="col-sm-4">@lang('admin.request.pickup_address') :</dt>
                            <dd class="col-sm-8">{{ $request->s_address ? $request->s_address : '-' }}</dd>

                            <dt class="col-sm-4">@lang('admin.request.drop_address') :</dt>
                            <dd class="col-sm-8">{{ $request->d_address ? $request->d_address : '-' }}</dd>
                        </dl>
                        @foreach($child_requests as $request_key => $child_request)
                            <dl class="row row-box">
                                <dt class="col-sm-4">@lang('admin.request.Provider_Name') :</dt>
                                @if($child_request->provider)
                                    <dd class="col-sm-8">{{ $child_request->provider->first_name }}</dd>
                                @else
                                    <dd class="col-sm-8">@lang('admin.request.provider_not_assigned')</dd>
                                @endif
                                @if($child_request->status == 'SCHEDULED')
                                    <dt class="col-sm-4">@lang('admin.request.ride_scheduled_time') :</dt>
                                    <dd class="col-sm-8">
                                        @if($child_request->schedule_at != "")
                                            {{ date('jS \of F Y h:i:s A', strtotime($child_request->schedule_at)) }}
                                        @else
                                            -
                                        @endif
                                    </dd>
                                @else
                                    <dt class="col-sm-4">@lang('admin.request.ride_start_time') :</dt>
                                    <dd class="col-sm-8">
                                        @if($child_request->started_at != "")
                                            {{ date('jS \of F Y h:i:s A', strtotime($child_request->started_at)) }}
                                        @else
                                            -
                                        @endif
                                    </dd>

                                    <dt class="col-sm-4">@lang('admin.request.ride_end_time') :</dt>
                                    <dd class="col-sm-8">
                                        @if($child_request->finished_at != "")
                                            {{ date('jS \of F Y h:i:s A', strtotime($child_request->finished_at)) }}
                                        @else
                                            -
                                        @endif
                                    </dd>
                                @endif

                                @if($child_request->payment)
                                    <dt class="col-sm-4">@lang('admin.request.base_price') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->fixed) }}</dd>
                                    @if($child_request->service_type->calculator=='MIN')
                                        <dt class="col-sm-4">@lang('admin.request.minutes_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->minute) }}</dd>
                                    @endif
                                    @if($child_request->service_type->calculator=='HOUR')
                                        <dt class="col-sm-4">@lang('admin.request.hours_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->hour) }}</dd>
                                    @endif
                                    @if($child_request->service_type->calculator=='DISTANCE')
                                        <dt class="col-sm-4">@lang('admin.request.distance_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->distance) }}</dd>
                                    @endif
                                    @if($child_request->service_type->calculator=='DISTANCEMIN')
                                        <dt class="col-sm-4">@lang('admin.request.minutes_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->minute) }}</dd>
                                        <dt class="col-sm-4">@lang('admin.request.distance_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->distance) }}</dd>
                                    @endif
                                    @if($child_request->service_type->calculator=='DISTANCEHOUR')
                                        <dt class="col-sm-4">@lang('admin.request.hours_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->hour) }}</dd>
                                        <dt class="col-sm-4">@lang('admin.request.distance_price') :</dt>
                                        <dd class="col-sm-8">{{ currency($child_request->payment->distance) }}</dd>
                                    @endif
                                    <dt class="col-sm-4">@lang('admin.request.commission') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->commision) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.fleet_commission') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->fleet) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.discount_price') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->discount) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.tax_price') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->tax) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.surge_price') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->surge) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.tips') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->tips) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.total_amount') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->total+$child_request->payment->tips) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.wallet_deduction') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->wallet) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.paid_amount') :</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->payable) }}</dd>

                                    <dt class="col-sm-4">@lang('admin.request.provider_earnings'):</dt>
                                    <dd class="col-sm-8">{{ currency($child_request->payment->provider_pay) }}</dd>

                                <!--  <dt class="col-sm-4">Provider Admin Commission :</dt>
                            <dd class="col-sm-8">{{ currency($child_request->payment->provider_commission) }}</dd> -->
                                @endif

                                <dt class="col-sm-4">@lang('admin.request.ride_status') : </dt>
                                <dd class="col-sm-8">
                                    {{ $child_request->status }}
                                </dd>
                            </dl>
                        @endforeach
                    </div>
                    <div class="col-md-6">
                        <div id="map"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@section('styles')
    <style type="text/css">
        #map {
            height: 450px;
        }
        .row-box {
            border: 1px solid #eee;
            padding: 10px 0px;
            margin-left: 0px;
            margin-right: 0px;
        }
    </style>
@endsection

@section('scripts')
    <script type="text/javascript">
        var map;
        var zoomLevel = 11;

        function initMap() {

            map = new google.maps.Map(document.getElementById('map'));

            var marker = new google.maps.Marker({
                map: map,
                icon: '/asset/img/marker-start.png',
                anchorPoint: new google.maps.Point(0, -29)
            });

            var markerSecond = new google.maps.Marker({
                map: map,
                icon: '/asset/img/marker-end.png',
                anchorPoint: new google.maps.Point(0, -29)
            });

            var bounds = new google.maps.LatLngBounds();

            source = new google.maps.LatLng({{ $request->s_latitude }}, {{ $request->s_longitude }});
            destination = new google.maps.LatLng({{ $request->d_latitude }}, {{ $request->d_longitude }});

            marker.setPosition(source);
            markerSecond.setPosition(destination);

            var directionsService = new google.maps.DirectionsService;
            var directionsDisplay = new google.maps.DirectionsRenderer({suppressMarkers: true, preserveViewport: true});
            directionsDisplay.setMap(map);

            directionsService.route({
                origin: source,
                destination: destination,
                travelMode: google.maps.TravelMode.DRIVING
            }, function(result, status) {
                if (status == google.maps.DirectionsStatus.OK) {
                    console.log(result);
                    directionsDisplay.setDirections(result);

                    marker.setPosition(result.routes[0].legs[0].start_location);
                    markerSecond.setPosition(result.routes[0].legs[0].end_location);
                }
            });

            @foreach($child_requests as $request_key => $child_request)
                @if($child_request->provider && $child_request->status != 'COMPLETED')
                    var markerProvider = new google.maps.Marker({
                        map: map,
                        icon: "/asset/img/marker-car.png",
                        anchorPoint: new google.maps.Point(0, -29)
                    });

                    provider = new google.maps.LatLng({{ $child_request->provider->latitude }}, {{ $child_request->provider->longitude }});
                    markerProvider.setVisible(true);
                    markerProvider.setPosition(provider);
                    console.log('Provider Bounds', markerProvider.getPosition());
                    bounds.extend(markerProvider.getPosition());
                @endif
            @endforeach

            bounds.extend(marker.getPosition());
            bounds.extend(markerSecond.getPosition());
            map.fitBounds(bounds);
        }
    </script>
    <script src="https://maps.googleapis.com/maps/api/js?key={{ Setting::get('map_key') }}&libraries=places&callback=initMap" async defer></script>
@endsection